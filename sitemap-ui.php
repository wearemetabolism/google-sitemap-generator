		...
		if(!empty($_REQUEST["sm_rebuild"])) { //Pressed Button: Rebuild Sitemap
			...
			foreach($this->sg->GetOptions() as $k=>$v) {
				...
				//Options of the category "Includes" are boolean
					...
					if( isset($_POST['sm_cf_customtypes'][$post_type]) )
						$this->sg->SetOption($k, $_POST['sm_cf_customtypes'][$post_type]);

				} else if(substr($k,0,17)=="sm_pr_customtypes" ) {
					$post_type = str_replace('sm_pr_customtypes', '', $k);
					$post_type = str_replace('[', '', str_replace(']', '', $post_type));

					if( isset($_POST['sm_pr_customtypes'][$post_type]) )
						$this->sg->SetOption($k, $_POST['sm_pr_customtypes'][$post_type]);
				...
				?>
					...
					<?php $this->HtmlPrintBoxHeader('sm_change_frequencies',__('Change Frequencies', 'sitemap')); ?>
						...
						<ul>
							...
							foreach ($custom_post_types as $post_type) {
								$post_type_object = get_post_type_object($post_type);
								$key = 'cf_customtypes['.$post_type_object->name.']';
								?>
								<li>
					...
					<?php $this->HtmlPrintBoxHeader('sm_priorities',__('Priorities', 'sitemap')); ?>
						<ul>
							...
							foreach ($custom_post_types as $post_type) {
								$post_type_object = get_post_type_object($post_type);
								$key = 'pr_customtypes['.$post_type_object->name.']';
								?>
								<li>
									<label for="sm_<?php echo $key ?>">
